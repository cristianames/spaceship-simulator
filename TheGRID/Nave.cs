...
namespace AlumnoEjemplos.TheGRID
{
    public class Nave : Dibujable
    {
        ...
        public Vector3 puntoLuzIzq()
        {
            ...
            return Vector3.Add(Vector3.Add(Vector3.Add(getPosicion(), Vector3.Multiply(getDireccion_X(), -9)),
                Vector3.Multiply(getDireccion_Y(), 0)), Vector3.Multiply(getDireccion(), 15));      //Siendo X=-9, Y=0, Z=15
        }
        public Vector3 puntoLuzDer()
        {
            return Vector3.Add(Vector3.Add(Vector3.Add(getPosicion(), Vector3.Multiply(getDireccion_X(), 9)),
                Vector3.Multiply(getDireccion_Y(), 2)), Vector3.Multiply(getDireccion(), 15));      //Siendo X=9, Y=2, Z=15
        }
        public Vector3 puntoLuzCent()
        {
            return Vector3.Add(Vector3.Add(Vector3.Add(getPosicion(), Vector3.Multiply(getDireccion_X(), 0)),
                Vector3.Multiply(getDireccion_Y(), 10)), Vector3.Multiply(getDireccion(), 25));      //Siendo X=0, Y=10, Z=25
        }
